...
export function Room() {
  ...
  useEffect(() => {
    ...
    // Hide any mesh containing 'techinz' in its name (even if nested)
    roomGLTF.scene.traverse((child) => {
      if (child.name && child.name.toLowerCase().includes("techinz")) {
        child.visible = false;
      }
    });
  ...
  return (
    <group>
      {/* Black plane to cover 'techinz.dev' text above monitor */}
      <mesh position={[0, 2.7, -2.2]} rotation={[-Math.PI / 2, 0, 0]}>
